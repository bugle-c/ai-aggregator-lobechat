...
const genProviderTable = (data: { desc?: string; id: string; name: string }) => {
  const title = genLink(data.name, urlJoin('https://gptweb.ru/discover/provider', data.id));
  ...
  const newMd = updateReadme(
    ...
    [
      ...
      PROVIDER_LIST.slice(10)
        ...
        .join('\n'),
      '</details>',
      `> 📊 Total providers: ${genLink(`<kbd>**${PROVIDER_LIST.length}**</kbd>`, 'https://gptweb.ru/discover/providers')}`,
    ].join('\n\n'),
  );
