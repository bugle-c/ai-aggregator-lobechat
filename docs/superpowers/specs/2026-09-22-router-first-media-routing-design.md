...
Смоук-тест видео `veo-free/seedance`: запущен 20:14 MSK, результат вписать сюда после завершения (длительность ролика, разрешение, соотношение сторон, время ответа) — это входные данные для таблицы профилей §3.3.

**Честный вывод по деньгам:** при текущих объёмах прямая экономия — порядка **250–300 ₽/мес**. Ценность идеи не в экономии сегодня, а в двух вещах: (а) первое впечатление новичка — Nano Banana вместо flux-schnell при нулевой себестоимости, (б) возможность позже открыть видео шире (Free/дешёвые тарифы) не платя WaveSpeed. Поэтому порядок реализации — **картинки → видео**, и видео только после измерений §5 Ф0.
...
**Ф3 — видео (1–2 дня, только если Ф0 ок).** `async/video.ts#createViaRouter`, `processVideoForGeneration(Buffer)`, семафор, учёт `routerStartedAt` в кронах, `ROUTER_FIRST_MEDIA=image,video`. Канареечно: сначала только `seedance` (дешевле ошибаться), потом `veo`.
