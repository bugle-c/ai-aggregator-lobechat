...
**Статус реализации (2026-09-23):** Ф1–Ф3 и «одно видео на Free» + «Оживить картинку» — в проде (`canary` `f074bc7f5c`, `45c67d301b`, `76dd6a5b60`); отличия от текста: право на видео считается по задачам (без колонки), новичковый дефолт картинок = для всех Free ≤ 7 дней, роутерный путь картинок — для всех (`ROUTER_FIRST_IMAGE_SCOPE=all`), видео через пул — для всех тарифов из `ROUTER_FIRST_VIDEO_PLANS` с бюджетом 3/день. Подробности — `KNOWLEDGE.md` §Router-first.
