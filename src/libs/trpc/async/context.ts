...
export const createAsyncRouteContext = async (request: NextRequest): Promise<AsyncContext> => {
  ...
  log('WebGPT auth header present: %s', !!lobeChatAuthorization);
  ...
  if (!lobeChatAuthorization) {
    log('No WebGPT authorization header found');
    throw new Error('No WebGPT authorization header found');
  }
  ...
  try {
    ...
    log('Decrypting WebGPT authorization');
