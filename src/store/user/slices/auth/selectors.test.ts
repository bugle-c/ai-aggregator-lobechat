...
describe('userProfileSelectors', () => {
  describe('displayUserName', () => {
    ...
    it('should return email when signed in but username is not existed in UserStore', () => {
      const store: UserStore = {
        isSignedIn: true,
        user: { email: 'demo@gptweb.ru' },
      } as UserStore;

      expect(userProfileSelectors.displayUserName(store)).toBe('demo@gptweb.ru');
    });
  ...
  describe('email', () => {
    it('should return user email if exist', () => {
      ...
      expect(userProfileSelectors.email(store)).toBe('demo@gptweb.ru');
    });
