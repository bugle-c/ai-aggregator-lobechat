...
export const POST = checkAuth(async (req: Request, { params, userId, serverDB }) => {
  ...
  try {
    ...
    if (modelId) {
      const { isModelAllowedForPlanAsync, getRequiredPlanForModelAsync } = await import(
        '@/server/modules/billing/model-tiers'
      );
    ...
    if (response instanceof Response) {
      ...
      (async () => {
        try {
          ...
          const { decideChargeAfterStream } = await import(
            '@/server/modules/billing/decideChargeAfterStream'
          );
