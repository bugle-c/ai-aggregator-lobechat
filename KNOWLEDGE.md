...
- **Credit holds are released exactly once (2026-09-23):** `server/modules/billing/hold-release.ts#releaseHoldIfActive` (`UPDATE … WHERE released_at IS NULL RETURNING`) gates every refund/reconcile in image/video `chargeAfterGenerate`; a second finisher logs «already released» and skips. Any cron that takes over an in-flight task must act only after the primary's deadline (`routerDeadlineAt + 60 s`) — the two racing 3 s apart double-refunded a 1080-credit hold.
- **Container restarts orphan in-process polls (2026-09-23):** every deploy recreates `lobehub`; the async video procedure polls the router in-process, so a task caught mid-poll would sit in `processing` for the 1 h timeout. Now `routerJobId`/`route`/`routerDeadlineAt` are written to task metadata right after submit and `poll-stuck-video-jobs` takes over `servedBy: llm-router-attempt` tasks via `media-router/finalizeVideo.ts` (done → asset+charge; running → wait until deadline+2 min; else Error + refund). Same idea applies to any future in-process wait.
