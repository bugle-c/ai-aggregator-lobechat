...
- **Not done (by owner's choice):** the "one video on Free" unlock and the «Оживить картинку» UI — Free video stays closed; the router video path serves paid plans (and free if ever allowed) per `ROUTER_FIRST_VIDEO_PLANS`.
- **Gotchas:** `user_billing.is_admin_granted` is NOT in the drizzle schema (admin migration only) — plan slug check covers it. Router image renders can take > 60 s when the pool is busy (deadline 90 s, then WaveSpeed). `writeUsageLog.test.ts` has 2 pre-existing failures unrelated to this (markup math).
