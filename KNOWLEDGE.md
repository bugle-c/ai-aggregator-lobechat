...
- **Gate:** `checkUsageLimit(db, userId, modelId, {kind})` → pure `decideUsageLimit()` (`checkUsageLimit.ts`). Order: (1) monthly pool `token_limit + token_balance + bonus` exhausted → `reason='monthly_cap'` (free, no paid pools) / `'credits'`; (2) free + `kind='chat'`: `count(*) usage_logs WHERE kind='chat' AND user_id AND created_at >= 00:00 MSK` (`daily-quota.ts`, `FREE_DAILY_MESSAGE_QUOTA=5`, MSK = fixed UTC+3) ≥ 5 → `reason='daily_quota'` **unless** credits above the allowance remain (`totalAvailable - max(tokensUsedMonth, creditLimit) > 0` — top-ups/bonus pools bypass the quota on the existing credit path); (3) paid plans and image/video (`kind` passed by both `chargeBeforeGenerate`) never see the quota. Counting unit = charged chat completions (rows written by `recordTokenUsage`; empty-stream/zero-cost answers don't count). Credits/`usage_logs` accounting unchanged.
- **DB:** `ai_aggregator.plans.token_limit` for `free` → **150** (monthly safety cap; code works with 30 too — the daily quota binds first). The bot's zero-credit guard still JOINs the frozen LobeChat `billing_plans` (free=35) → keep that row in sync or bot users hit «Кредиты закончились» at 35. No new index: `usage_logs_user_created_idx`/`usage_logs_created_idx` serve the count (EXPLAIN 0.09 ms).
...
- **Bot mirror:** `gptwebrubot/src/lobechat/dailyQuota.ts` duplicates the constant + copy; `streamChat` parses the refusal body so TG users see the paywall copy instead of `HTTP 500: {json}`.
