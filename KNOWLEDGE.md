...
- **FlowSidebar is a scrolling flex column → every slot `flex-shrink: 0` (2026-09-07).** Without it a column taller than the viewport (style card + prompt + open «Ещё настройки») was *shrunk* by the browser instead of scrolled: the style card's media box collapsed to 0 px and the owner read it as «настройки открылись отдельной страничкой». The CTA is `position: sticky; bottom` inside the scroller. Locked knobs in the advanced panel render as one compact row (name · 🔒 · reason) — the unusable control is not mounted.
